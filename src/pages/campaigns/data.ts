...
const RAW: RawCampaign[] = [
  { name: "Масажер для шиї Neck Relax", active: true, leads: 1840, spend: 73600, approves: 712, impressions: 412000, clicks: 9100, avgCheck: 1290, margin: 520, cogs: 410, buyout: 0.66, buyerPct: 0.3 },
  { name: "Робот-пилосос CleanMax X9", active: true, leads: 1320, spend: 92400, approves: 488, impressions: 366000, clicks: 6700, avgCheck: 3490, margin: 1180, cogs: 1620, buyout: 0.59, buyerPct: 0.25 },
  { name: "Тример для бороди BarberPro", active: true, leads: 2410, spend: 67480, approves: 1010, impressions: 540000, clicks: 13200, avgCheck: 890, margin: 360, cogs: 240, buyout: 0.71, buyerPct: 0.3 },
  { name: "Корсет для постави PostureFix", active: true, leads: 980, spend: 31360, approves: 402, impressions: 248000, clicks: 5400, avgCheck: 740, margin: 310, cogs: 150, buyout: 0.63, buyerPct: 0.35 },
  { name: "Смарт-годинник FitWatch 7", active: true, leads: 1660, spend: 116200, approves: 560, impressions: 470000, clicks: 8900, avgCheck: 1990, margin: 690, cogs: 920, buyout: 0.55, buyerPct: 0.25 },
  { name: "Зволожувач повітря AromaMist", active: false, leads: 540, spend: 16740, approves: 196, impressions: 142000, clicks: 3100, avgCheck: 990, margin: 380, cogs: 300, buyout: 0.6, buyerPct: 0.3 },
  { name: "Набір кухонних ножів SharpEdge", active: true, leads: 1120, spend: 39200, approves: 470, impressions: 286000, clicks: 6100, avgCheck: 1150, margin: 470, cogs: 360, buyout: 0.68, buyerPct: 0.3 },
  { name: "Лампа-нічник MoonLight", active: true, leads: 760, spend: 19000, approves: 318, impressions: 198000, clicks: 4600, avgCheck: 640, margin: 280, cogs: 120, buyout: 0.64, buyerPct: 0.35 },
  { name: "Електросушарка для взуття DryStep", active: false, leads: 410, spend: 11070, approves: 158, impressions: 108000, clicks: 2400, avgCheck: 590, margin: 250, cogs: 110, buyout: 0.57, buyerPct: 0.3 },
  { name: "Масажний пістолет PulseGun", active: true, leads: 1290, spend: 90300, approves: 442, impressions: 352000, clicks: 7200, avgCheck: 2290, margin: 820, cogs: 1080, buyout: 0.58, buyerPct: 0.25 },
  { name: "Органайзер для авто CarTidy", active: true, leads: 880, spend: 22000, approves: 372, impressions: 210000, clicks: 5000, avgCheck: 690, margin: 300, cogs: 130, buyout: 0.69, buyerPct: 0.35 },
  { name: "Грілка USB WarmHands", active: false, leads: 330, spend: 8910, approves: 124, impressions: 96000, clicks: 2100, avgCheck: 540, margin: 230, cogs: 90, buyout: 0.61, buyerPct: 0.3 },
]
...
function compute(r: RawCampaign): Computed {
  ...
  const ccCost = approves * s.ccPerOrder + r.margin * approves * s.ccUpsellPct * 0.15
  ...
  const probableIncome = marginBuyout - (spend + returns + ccCost + packaging + cogsTotal)
  ...
  const breakEvenLeadPrice = (marginBuyout - (returns + ccCost + packaging + cogsTotal)) / leads
...
const PORTFOLIO_OF = ["p1", "p1", "p1", "p2", "p1", "p3", "p2", "p1", "p3", "p1", "p2", "p3"]
...
const PLATFORM_OF: PlatformId[] = ["facebook", "google", "facebook", "tiktok", "facebook", "tiktok", "google", "facebook", "google", "facebook", "tiktok", "google"]
...
function scaleRaw(r: RawCampaign, frac: number, overrides?: Partial<RawCampaign>): RawCampaign {
  return {
    ...r,
...
RAW.forEach((r, ci) => {
  ...
  gFracs.forEach((gf, gi) => {
    ...
    aFracs.forEach((af, ai) => {
      ...
      aRaw.name = GROUP_LABELS[gi] + " · " + AD_LABELS[(gi + ai) % AD_LABELS.length]
      ADS.push({
        ...compute(aRaw),
        portfolio: PORTFOLIO_OF[ci],
        platform: PLATFORM_OF[ci],
...
export const COLUMNS: Column[] = [
  { key: "leads", label: "Ліди", unit: "", group: "perf", hint: "Кількість лідів з рекламного кабінету" },
  { key: "costPerLead", label: "Ціна ліда", unit: "₴", group: "perf", hint: "Сума витрат / кількість лідів" },
  { key: "approves", label: "Апрув", unit: "", group: "perf", hint: "Підтверджені замовлення з певних статусів СРМ" },
  { key: "approveRate", label: "% апруву", unit: "%", group: "perf", hint: "Апрув / ліди × 100" },
  { key: "costPerApprove", label: "Ціна апруву", unit: "₴", group: "perf", hint: "Сума витрат / кількість апрувів" },
  { key: "spend", label: "Сума витрат", unit: "₴", group: "money", hint: "Загальна сума витрат на рекламу (ADS)" },
  { key: "breakEvenLeadPrice", label: "Ціна ліда в 0", unit: "₴", group: "derived", hint: "Ціна ліда для виходу в нуль" },
  { key: "cpm", label: "CPM", unit: "₴", group: "perf", hint: "Ціна за 1000 показів (ADS)" },
  { key: "cpc", label: "CPC", unit: "₴", group: "perf", hint: "Ціна за клік (ADS)" },
  { key: "ctr", label: "CTR", unit: "%", group: "perf", hint: "Клікабельність (ADS)" },
  { key: "buyoutRate", label: "% викупу", unit: "%", group: "money", hint: "Викуплені / апрувнуті × 100" },
  { key: "returns", label: "Повернення", unit: "₴", group: "money", hint: "Відмови × ціна повернення (ТТН)" },
  { key: "ccCost", label: "Витрати на КЦ", unit: "₴", group: "money", hint: "Обробка замовлень + % від маржі допродажу" },
  { key: "packaging", label: "Пакування", unit: "₴", group: "money", hint: "Витрати на склад / пакування посилок" },
  { key: "probableIncome", label: "Ймовірний дохід", unit: "₴", group: "derived", hint: "Маржа викупу − усі супутні витрати" },
  { key: "buyerIncome", label: "Дохід баєра", unit: "₴", group: "derived", hint: "Ймовірний дохід × % ЗП баєра" },
  { key: "ownerIncome", label: "Дохід овнера", unit: "₴", group: "derived", owner: true, hint: "Ймовірний дохід − дохід баєра" },
  { key: "roi", label: "ROI", unit: "%", group: "derived", emphasize: true, hint: "Рентабельність з урахуванням собівартості" },
  { key: "romi", label: "ROMI", unit: "%", group: "derived", emphasize: true, hint: "Рентабельність без собівартості товару" },
]
...
export function fmt(val: number | null | undefined, unit: Column["unit"]) {
  ...
  if (unit === "%") return val.toLocaleString("uk", { maximumFractionDigits: 2 }) + "%"
  if (!Number.isInteger(val)) return val.toLocaleString("uk", { maximumFractionDigits: 2 })
  return groupNum(val)
}
